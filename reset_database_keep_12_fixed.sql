...
-- Supprimer les agapes liées aux sessions à supprimer
DELETE FROM agape WHERE session_id IN (SELECT id FROM temp_sessions_to_delete);

-- Supprimer les remboursements liés aux sessions àBAADprimer
DELETE FROM refund WHERE session_id IN (SELECT id FROM temp_sessions_to_delete);

-- Supprimer les relations emprunt-épargne pour les emprunts à supprimer
...
-- Supprimer les emprunts liés aux sessions à supprimer
DELETE FROM borrowing WHERE session_id IN (SELECT id FROM temp_sessions_to_delete);

-- Supprimer les épargnes liées aux sessions à supprimer
...
-- ============================================
-- Vérification: Compter les seBAADns restantes
-- ============================================
SELECT 
    e.id AS exercise_id,
    e.year,
...
SELECT 
    ...
    s.created_at
FROM session s
WHERE s.exercise_id = (SELECT id FROM exercise WHERE active = 1 LIMIT 1)
